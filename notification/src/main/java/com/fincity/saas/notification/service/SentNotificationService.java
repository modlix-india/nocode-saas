...
public class SentNotificationService
    ...
    private final Map<
                    ...
                    IChannelStorageService<ULong, ? extends AbstractUpdatableDTO<ULong, ULong>, ?>>
            storageServices = new EnumMap<>(NotificationChannelType.class);
    ...
    public Mono<SendRequest> toPlatformNotification(
            SendRequest request, NotificationDeliveryStatus deliveryStatus, NotificationChannelType... channelTypes) {
    ...
    public Mono<SendRequest> toGatewayNotification(
            SendRequest request, NotificationDeliveryStatus deliveryStatus, NotificationChannelType... channelTypes) {
    ...
    public Mono<SendRequest> toNetworkNotification(
            SendRequest request, NotificationDeliveryStatus deliveryStatus, NotificationChannelType... channelTypes) {
    ...
    private Mono<SentNotification> createFromRequest(
            SendRequest request,
        ...
        return FlatMapUtil.flatMapMono(
                        () -> this.createSentNotification(
                                request, triggerTime, notificationStage, deliveryStatus, channelTypes),
                        this::create,
                        (sendNotification, created) ->
                                this.createChannelStorage(request, triggerTime, notificationStage, deliveryStatus),
    ...
    private Mono<SentNotification> updateOrCreateFromRequest(
            SendRequest request,
            NotificationStage notificationStage,
            NotificationDeliveryStatus deliveryStatus,
            NotificationChannelType... channelTypes) {
        ...
        return FlatMapUtil.flatMapMono(
                        ...
                        (req, sentNotification) -> this.updateSentNotification(
                                sentNotification, req, triggerTime, notificationStage, deliveryStatus, channelTypes),
                        (req, sentNotification, updatedNotification) -> this.update(updatedNotification),
                        (req, sentNotification, updatedNotification, updated) ->
                                this.updateChannelStorage(request, triggerTime, notificationStage, deliveryStatus),
                        (req, sentNotification, updatedNotification, updated, sUpdated) ->
    ...
    private Mono<SentNotification> createSentNotification(
            SendRequest request,
            LocalDateTime createdTime,
            NotificationStage notificationStage,
            NotificationDeliveryStatus deliveryStatus,
            NotificationChannelType... channelTypes) {
        return this.updateStageAndStatus(
                SentNotification.from(request, createdTime),
                request,
                createdTime,
                notificationStage,
                deliveryStatus,
    ...
    private Mono<SentNotification> updateSentNotification(
            ...
            NotificationChannelType... channelTypes) {
        return this.updateStageAndStatus(
                sentNotification, request, createdTime, notificationStage, deliveryStatus, channelTypes);
    }

    private Mono<SentNotification> updateStageAndStatus(
            SentNotification sentNotification,
            SendRequest request,
            LocalDateTime createdTime,
            NotificationStage notificationStage,
        ...
        if (notificationStage != null) sentNotification.setNotificationStage(notificationStage);

        if (deliveryStatus == null || request.isEmpty()) return Mono.just(sentNotification);

        this.updateErrorStatus(sentNotification, request, deliveryStatus);

        List<NotificationChannelType> enabledChannels = deliveryStatus.isError()
                ? request.getErrorChannels()
    ...
    private void updateErrorStatus(
            SentNotification sentNotification, SendRequest request, NotificationDeliveryStatus deliveryStatus) {

        if (deliveryStatus.isError()) {
            if (request.getErrorInfo() != null) sentNotification.setErrorInfo(request.getErrorInfo());

            if (request.getChannelErrors() != null
                    && !request.getChannelErrors().isEmpty())
                sentNotification.setChannelErrorInfo(request.getChannelErrors());
        }
    }

    private Mono<Boolean> createChannelStorage(
            SendRequest request,
    ...
    private Mono<Boolean> updateChannelStorage(
            SendRequest request,
            LocalDateTime triggerTime,
            NotificationStage notificationStage,
            NotificationDeliveryStatus deliveryStatus) {
    ...
    private boolean isChannelAvailable(
            NotificationChannelType channelType, SendRequest request, NotificationDeliveryStatus deliveryStatus) {
        return deliveryStatus.isError()
                ? request.getChannelErrors().containsKey(channelType.getLiteral())
                : request.getChannels().containsChannel(channelType);
    }
}
