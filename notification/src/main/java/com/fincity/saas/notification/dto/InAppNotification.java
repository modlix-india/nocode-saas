...
public class InAppNotification extends AbstractUpdatableDTO<ULong, ULong> implements MultiAction<InAppNotification> {
	...
	public static InAppNotification from(SendRequest request, LocalDateTime triggerTime) {
		...
		if (request == null || !request.getChannels().containsChannel(NotificationChannelType.IN_APP))
			return null;
	...
	public InAppNotification setNotificationDeliveryStatus(NotificationDeliveryStatus status,
