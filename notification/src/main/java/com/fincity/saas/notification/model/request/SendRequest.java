...
public class SendRequest implements Serializable {
        ...
        return new SendRequest()
                ...
                .setNotificationType(
                        notificationType != null
                                ? NotificationType.lookupLiteral(notificationType)
    ...
    public static SendRequest ofError(
            String appCode, String clientCode, BigInteger userId, String notificationType, GenericException errorInfo) {
    ...
    public boolean isValid(NotificationChannelType channelType) {
        return this.connections != null
                && this.channels != null
                && this.connections.containsKey(channelType.getLiteral())
                && this.channels.containsChannel(channelType);
    }

    @JsonIgnore
    public boolean isEmpty() {
        if (this.channels == null) return true;

        return !this.channels.containsAnyChannel();
    }

    public <T extends GenericException> SendRequest setChannelErrorInfo(
            T exception, NotificationChannelType channelType) {
        if (this.channelErrors == null) this.channelErrors = new HashMap<>();
    ...
    public boolean isError(NotificationChannelType channelType) {
        return this.errorInfo != null
                || ((this.channelErrors != null && !this.channelErrors.isEmpty())
                        && this.channelErrors.containsKey(channelType.getLiteral()));
    }

    @JsonIgnore
    public List<NotificationChannelType> getErrorChannels() {
        if (this.channelErrors == null || this.channelErrors.isEmpty()) return List.of();

        return this.channelErrors.keySet().stream()
                .map(NotificationChannelType::lookupLiteral)
                .toList();
    }
}
