...
public class InAppNotificationService
        extends AbstractCodeService<
                NotificationInAppNotificationsRecord, ULong, InAppNotification, InAppNotificationDao>
    ...
    public Mono<InAppNotification> saveChannelNotification(
        ...
        return this.updateStageAndStatus(
                        InAppNotification.from(request, triggerTime), triggerTime, notificationStage, deliveryStatus)
                .flatMap(this::create)
                .contextWrite(Context.of(LogUtil.METHOD_NAME, "InAppNotificationService.saveChannelNotification"));
    }

    @Override
    public Mono<InAppNotification> updateChannelNotification(
            SendRequest request,
        ...
        return FlatMapUtil.flatMapMono(
                        ...
                        inAppNotification -> this.updateStageAndStatus(
                                inAppNotification, createdTime, notificationStage, deliveryStatus),
                        (inAppNotification, uInAppNotification) -> this.update(uInAppNotification),
                        (inAppNotification, uInAppNotification, updated) ->
                                this.evictCode(updated.getCode()).map(evicted -> updated))
    ...
    private Mono<InAppNotification> updateStageAndStatus(
            InAppNotification inAppNotification,
            LocalDateTime createdTime,
            NotificationStage notificationStage,
            NotificationDeliveryStatus deliveryStatus) {

        if (inAppNotification == null) return Mono.empty();

        if (notificationStage != null) inAppNotification.setNotificationStage(notificationStage);
