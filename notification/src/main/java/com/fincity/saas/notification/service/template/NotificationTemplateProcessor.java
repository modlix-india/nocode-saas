...
public class NotificationTemplateProcessor extends BaseTemplateProcessor {
    ...
    private <T extends NotificationMessage<T>> T getNotificationMessage(
            NotificationChannelType channelType, Map<String, String> templatePart) {

        if (channelType == null || channelType == NotificationChannelType.DISABLED) return null;
    ...
    public <T extends NotificationMessage<T>> Mono<T> process(
            NotificationChannelType channelType, NotificationTemplate template, Object templateData) {
        return this.process(null, channelType, template, templateData);
    }

    public <T extends NotificationMessage<T>> Mono<T> process(
            String language, NotificationChannelType channelType, NotificationTemplate template, Object templateData) {

        if (template.getTemplateParts().isEmpty() || StringUtil.safeIsBlank(template.getCode()))
            return this.msgService.throwMessage(
                    TemplateProcessingException::new,
                    "$ info not provided for $ template in $ Channel.",
                    "Template",
                    template.getCode(),
        ...
        return FlatMapUtil.flatMapMono(
                        ...
                        lang -> {
                            Map<String, String> templatePart = template.getTemplateParts()
                                    .getOrDefault(
                        ...
                        (lang, notificationMessage) -> this.processUserInfo(
                                notificationMessage.getChannelType(),
                                template.getCode(),
                                templateData,
                                template.getRecipientExpressions()),
                        (lang, notificationMessage, userInfo) ->
                                this.processInternal(userInfo, template.getCode(), notificationMessage, templateData))
    ...
    private <T extends NotificationMessage<T>> Mono<T> processInternal(
        ...
        return Mono.zip(
                        super.processString(
                                this.getSubjectName(templateCode), notificationMessage.getSubject(), templateData),
                        super.processString(
                                this.getBodyName(templateCode), notificationMessage.getBody(), templateData))
                .map(message -> notificationMessage
                        .updateMessage(message.getT1(), message.getT2())
                        .addRecipientInfo(userInfo));
    }

    public Mono<RecipientInfo> processUserInfo(
        ...
        if (recipientTypes.isEmpty()) return Mono.just(new RecipientInfo());
    ...
    protected Mono<String> getRecipient(
            String templateCode, String recipient, String recipientExpression, Object templateData) {
    ...
    protected Mono<String> getEffectiveLanguage(
            String requestedLanguage, NotificationTemplate template, Object templateData) {
        return !StringUtil.safeIsBlank(requestedLanguage)
                ? Mono.just(requestedLanguage)
    ...
    protected Mono<String> getLanguage(NotificationTemplate template, Object templateData) {
        ...
        if (StringUtil.safeIsBlank(template.getLanguageExpression())) return Mono.just(defaultLanguage);

        return super.processString(
                        this.getLanguageName(template.getCode()),
                        template.getLanguageExpression(),
                        templateData,
                        defaultLanguage)
                .map(lang ->
                        StringUtil.safeIsBlank(lang) ? this.getDefaultLanguage(template.getDefaultLanguage()) : lang)
    ...
    private List<String> getAllTemplateName(NotificationChannelType channelType, String templateCode) {
        ...
        channelType
                ...
                .forEach(recipientType -> names.add(this.getRecipientName(templateCode, recipientType.getLiteral())));
