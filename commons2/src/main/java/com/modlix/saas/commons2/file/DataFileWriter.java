package com.modlix.saas.commons2.file;
...
import com.modlix.saas.commons2.util.DataFileType;
...
public class DataFileWriter implements AutoCloseable, Flushable {
	...
	public DataFileWriter(List<List<String>> preHeaders, List<String> header, DataFileType fileType,
	        OutputStream stream, boolean writeHeader) {
		this.header = header;
		this.preHeaders = preHeaders;
		this.os = stream;
		this.writeHeader = writeHeader;
		this.fileType = fileType;
		if (this.fileType == DataFileType.JSON || this.fileType == DataFileType.JSONL)
			this.gson = new Gson();
	}
	...
	public void write(Map<String, Object> map) throws IOException {
		...
		if (!this.fileType.isNestedStructure())
			this.writeLineForFlatFile(map);
		else
			this.writelineForNestedFile(map);
	...
	private void writelineForNestedFile(Map<String, Object> map) throws IOException {
		...
		if (!this.firstLine)
			this.os.write((this.fileType == DataFileType.JSON ? ",\n" : "\n").getBytes());

		this.os.write(this.gson.toJson(map)
		        .getBytes());
	}
...
<Table>""").getBytes());
	...
	protected void writeLine(List<String> line) throws IOException {
		...
		if (this.fileType == DataFileType.CSV)
			...
			this.os.write(line.stream()
			        .collect(Collectors.joining("</Data></Cell><Cell><Data ss:Type=\"String\">",
			                "<Row><Cell><Data ss:Type=\"String\">", "</Data></Cell></Row>"))
			        .getBytes());
		else if (this.fileType == DataFileType.XLSX)
			this.zos.write(line.stream()
			        .collect(Collectors.joining("</t></is></c><c t=\"inlineStr\"><is><t>",
	...
	public String singleColumnConvert(String e, char delim) {
		...
		if (hasDoubleQuotes)
			str = e.replace("\"", "\"\"");
		boolean surround = hasDoubleQuotes || (e.indexOf(delim) != -1) || (e.indexOf('\n') != -1)
		        || (e.indexOf('\r') != -1);
		if (!surround)
			return str;
		return '"' + str + '"';
	...
	protected void writeLineForFlatFile(Map<String, Object> map) throws IOException {
		...
		this.writeLine(this.header.stream()
		        .map(e -> com.modlix.saas.commons2.util.StringUtil.safeValueOf(map.get(e), ""))
		        .toList());
	}
