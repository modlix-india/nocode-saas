...
CREATE TABLE `message`.`message_exotel_calls` (
    ...
    `USER_ID` BIGINT UNSIGNED NULL COMMENT 'ID of the user associated with this Exotel Call.',
    ...
    `FROM` CHAR(15) NOT NULL COMMENT 'Phone number of the caller.',
    `TO_DIAL_CODE` SMALLINT NOT NULL DEFAULT 91 COMMENT 'Dial code of the receiver\'s phone number.',
    `TO` CHAR(15) NOT NULL COMMENT 'Phone number of the receiver.',
    `CALLER_ID` CHAR(50) NULL COMMENT 'Caller ID configured in Exotel.',
    `EXOTEL_CALL_STATUS` ENUM ('QUEUED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'BUSY', 'NO_ANSWER') DEFAULT 'IN_PROGRESS' NOT NULL COMMENT 'Status of the call.',
...
CREATE TABLE `message`.`message_calls` (
    ...
    `CALL_STATUS` ENUM ('UNKNOWN', 'QUEUED', 'ORIGINATE', 'FAILED', 'BUSY', 'NO_ANSWER', 'CALL_COMPLETE', 'INSUFFICIENT_BALANCE', 'CANCELED') NOT NULL DEFAULT 'UNKNOWN' COMMENT 'Status of the call.',
