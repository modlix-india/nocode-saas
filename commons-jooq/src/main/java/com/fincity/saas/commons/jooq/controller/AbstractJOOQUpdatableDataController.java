...
public class AbstractJOOQUpdatableDataController<
    ...
    public Mono<ResponseEntity<D>> put(
        ...
        return this.service.update(entity).map(ResponseEntity::ok);
    }

    @PatchMapping(AbstractJOOQDataController.PATH_ID)
    public Mono<ResponseEntity<D>> patch(
            @PathVariable(name = PATH_VARIABLE_ID) final I id, @RequestBody Map<String, Object> entityMap) {
        return this.service.update(id, entityMap).map(ResponseEntity::ok);
    }
}
