...
public class SendResponse<T extends NotificationMessage<T>> implements Serializable {
    ...
    private static <T extends NotificationMessage<T>> SendResponse<T> of(
            String code,
            String appCode,
            String clientCode,
            BigInteger userId,
    ...
    public static <T extends NotificationMessage<T>> SendResponse<T> of(
            SendRequest request, NotificationChannelType channelType) {

        if (!request.getChannels().containsChannel(channelType)) return null;
        ...
        return of(
                ...
                (T) request.getChannels().get(channelType));
    }
