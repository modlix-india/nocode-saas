...
public enum ActivityAction implements EnumType {
    ...
    TAG_CREATE("TAG_CREATE", "$entity was tagged $%s by $user.".formatted(Ticket.Fields.tag),
            keys(Ticket.Fields.tag)),
    TAG_UPDATE("TAG_UPDATE", "$entity was tagged $%s from $%s by $user.".formatted(Ticket.Fields.tag, ActivityAction.getOldName(Ticket.Fields.tag)),
