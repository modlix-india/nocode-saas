...
public class NoteService extends BaseContentService<EntityProcessorNotesRecord, Note, NoteDAO>
    ...
    private void init() {
        ...
        String noteSchemaRef = SchemaUtil.getNamespaceForClass(Note.class) + "." + Note.class.getSimpleName();
        SchemaUtil.ArgSpec<NoteRequest> noteRequest = SchemaUtil.ArgSpec.ofRef("noteRequest", NoteRequest.class);
    ...
    public Mono<ReactiveRepository<Schema>> getSchemaRepository(
        ...
        try {
            ...
            if (schema != null) {
                ...
                LOGGER.info("Generated schema for Note class: {}.{}", namespace, name);
            }
        } catch (Exception e) {
            LOGGER.error("Failed to generate schema for Note class: {}", e.getMessage(), e);
        }
