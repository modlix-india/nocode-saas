...
public class TicketExpirationService {
    ...
    public TicketExpirationService(
            ProductTicketExRuleService productTicketExRuleService, TicketExpirationDAO ticketExpirationDAO) {
    ...
    public Mono<ExpireTicketsResult> runExpiration(ProcessorAccess access) {
        ...
        return FlatMapUtil.flatMapMono(() -> productTicketExRuleService.getActiveRules(access), rules -> {
                    List<ProductTicketExRule> sorted = rules.stream()
                            .sorted(Comparator.comparing(r -> r.getProductTemplateId() == null))
                            .toList();

                    return Flux.fromIterable(sorted)
                            .concatMap(rule -> {
                                LocalDateTime cutoff = now.minusDays(rule.getExpiryDays());
                                return ticketExpirationDAO.expireTicketsForRuleFully(
                ...
                .contextWrite(Context.of(LogUtil.METHOD_NAME, "TicketExpirationService.runExpiration"));
    }
}
