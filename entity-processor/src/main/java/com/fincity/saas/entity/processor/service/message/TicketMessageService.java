...
public class TicketMessageService {
    ...
    public Mono<Void> sendOnTicketCreate(ProcessorAccess access, Ticket ticket) {
        ...
        return this.configService
                ...
                .then()
                // Messaging must never fail ticket creation, but a swallowed error here is
                // indistinguishable from "no configs matched", so log before dropping it.
                .onErrorResume(e -> {
                    logger.error(
                            "Failed to dispatch on-create messages for ticket {} on product {}",
                            ticket.getId(),
                            ticket.getProductId(),
                            e);
                    return Mono.empty();
