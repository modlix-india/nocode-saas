DROP TABLE IF EXISTS `entity_processor`.`entity_processor_product_template_walk_in_forms`;

CREATE TABLE `entity_processor`.`entity_processor_product_template_walk_in_forms` (
    ...
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK1_PRODUCT_TEMPLATE_WALK_IN_FORMS_CODE` (`CODE`),
    UNIQUE KEY `UK2_PRODUCT_TEMPLATE_WALK_IN_FORMS_PRODUCT_TEMPLATE_ID` (`APP_CODE`, `CLIENT_CODE`, `PRODUCT_TEMPLATE_ID`),

    CONSTRAINT `FK1_PRODUCT_TEMPLATE_WALK_IN_FORMS_PRODUCT_ID`
        FOREIGN KEY (`PRODUCT_TEMPLATE_ID`)
            REFERENCES `entity_processor_product_templates` (`ID`)
            ON DELETE RESTRICT
            ON UPDATE CASCADE,

    CONSTRAINT `FK2_PRODUCT_TEMPLATE_WALK_IN_FORMS_STAGE_ID`
        FOREIGN KEY (`STAGE_ID`)
            REFERENCES `entity_processor_stages` (`ID`)
            ON DELETE RESTRICT
            ON UPDATE CASCADE,

    CONSTRAINT `FK3_PRODUCT_TEMPLATE_WALK_IN_FORMS_STATUS_ID`
