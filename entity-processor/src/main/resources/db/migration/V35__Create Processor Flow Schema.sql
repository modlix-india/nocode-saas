...
CREATE TABLE `entity_processor`.`entity_processor_flow_schema` (
    ...
    `DB_SCHEMA` CHAR(22) NOT NULL DEFAULT 'entity_processor' COMMENT 'Schema of DB for this flow Schema',
    `DB_TABLE_NAME` CHAR(128) NOT NULL COMMENT 'Name of the table in this this entity is present for flow Schema',
    `DB_ID` BIGINT UNSIGNED NULL COMMENT 'ID for entity for which this flow schema is created',
    ...
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK1_FLOW_SCHEMA_AC_CC_SCHEMA_TABLE_NAME_ID` (`APP_CODE`, `CLIENT_CODE`, `DB_SCHEMA`, `DB_TABLE_NAME`, `DB_ID`)
