-- Create product ticket expiration rules table
...
-- Add EXPIRES_ON column to tickets table

ALTER TABLE `entity_processor`.`entity_processor_tickets`
    ADD COLUMN `EXPIRES_ON` DATETIME NULL DEFAULT NULL
    COMMENT 'Ticket expiration datetime. Expired when EXPIRES_ON < NOW().'
    AFTER `AD_ID`;

-- Index for recalculateExpiresOn: filters by (AC, CC, SOURCE, PRODUCT_ID) with EXPIRES_ON range
ALTER TABLE `entity_processor`.`entity_processor_tickets`
    ADD INDEX `IDX6_TICKETS_AC_CC_SRC_PID_EXP` (`APP_CODE`, `CLIENT_CODE`, `SOURCE`, `PRODUCT_ID`, `EXPIRES_ON`);
