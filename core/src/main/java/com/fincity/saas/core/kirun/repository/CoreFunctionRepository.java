...
import com.fincity.saas.core.functions.notification.SendNotification;
...
import com.fincity.saas.core.functions.security.IsUserBeingManaged;
...
import com.fincity.saas.core.functions.storage.CreateManyStorageObject;
...
public class CoreFunctionRepository implements ReactiveRepository<ReactiveFunction> {
	...
	public CoreFunctionRepository(CoreFunctionRepositoryBuilder builder) {
		...
		this.makeFileFunctions(builder.filesService, builder.templateConversionService, builder.filesService,
				builder.gson);
		this.makeCryptoFunctions();
		this.makeHashingFunctions();
		this.makeNotificationFunctions(builder.notificationProcessingService);
	...
	private void makeStorageFunctions(AppDataService appDataService, ObjectMapper objectMapper, Gson gson) {
		...
		this.addToRepoMap(createStorage, createManyStorage, deleteStorage, deleteByFilterStorage, updateStorage,
				readStorage, readPageStorage);
	}
	...
	private void makeRESTFunctions(RestService restService, IFeignFilesService filesService,
		...
		ReactiveFunction getRequest = new CallRequest(restService, filesService, securityService, "GetRequest",
				"GET", false, gson);
		ReactiveFunction postRequest = new CallRequest(restService, filesService, securityService, "PostRequest",
				"POST", true, gson);
		ReactiveFunction putRequest = new CallRequest(restService, filesService, securityService, "PutRequest",
				"PUT", true, gson);
	...
	private void makeNotificationFunctions(NotificationProcessingService notificationProcessingService) {

		ReactiveFunction sendNotification = new SendNotification(notificationProcessingService);

		this.addToRepoMap(sendNotification);
	}

	private void addToRepoMap(ReactiveFunction... reactiveFunctions) {
		for (ReactiveFunction reactiveFunction : reactiveFunctions) {
			repoMap.put(reactiveFunction.getSignature().getFullName(), reactiveFunction);
		}
	}
