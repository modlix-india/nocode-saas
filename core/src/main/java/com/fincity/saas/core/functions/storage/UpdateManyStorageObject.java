...
public class UpdateManyStorageObject extends AbstractReactiveFunction
{
    private static final String DATA_ARRAY = "dataArray";

    private static final String EVENT_RESULT = "result";

    private static final String FUNCTION_NAME = "UpdateMany";
    ...
    public UpdateManyStorageObject(AppDataService appDataService, Gson gson) {
    ...
    protected Mono<FunctionOutput> internalExecute(ReactiveFunctionExecutionParameters context) {
        ...
        if (dataList.stream().anyMatch(dataObj -> !dataObj.containsKey("_id") || dataObj.get("_id") == null)) {
            return Mono.just(new FunctionOutput(List.of(EventResult.of(Event.ERROR, Map.of(Event.ERROR,
                    new JsonPrimitive("Each item must have a id to proceed with the update."))))));
        }
        ...
        return appDataService.updateMany(StringUtil.isNullOrBlank(appCode) ? null : appCode,
                        StringUtil.isNullOrBlank(clientCode) ? null : clientCode, storageName,
                        dataArr, eager, eagerFields, !isPartial)
