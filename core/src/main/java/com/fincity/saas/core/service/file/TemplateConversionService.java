...
public class TemplateConversionService {
    ...
    public Mono<byte[]> convert(String templateName, String appCode, String clientCode, String outputFormat,
        ...
        return FlatMapUtil.flatMapMono(
                ...
                (acTup, convService) -> templateService.read(templateName, acTup.getT1(), acTup.getT2())
                        .map(ObjectWithUniqueID::getObject)
                        .switchIfEmpty(msgService.throwMessage(msg -> new GenericException(HttpStatus.NOT_FOUND, msg),
                                CoreMessageResourceService.TEMPLATE_DETAILS_MISSING, templateName)),

                (acTup, convService, template) -> convService.convertTemplate(template, outputFormat, templateData))
