...
public class TemplateToPdf extends AbstractReactiveFunction {
	...
	public TemplateToPdf(TemplateConversionService templateConversionService, IFeignFilesService fileService,
			Gson gson) {
	...
	protected Mono<FunctionOutput> internalExecute(ReactiveFunctionExecutionParameters context) {
		...
		return Mono.deferContextual(cv -> {
			...
			return FlatMapUtil.flatMapMono(
					...
					acTup -> templateConversionService.convert(templateName, acTup.getT1(), acTup.getT2(),
							OUTPUT_FORMAT, templateData),
					(acTup, fileBytes) -> {
						if (fileBytes == null)
							return Mono.error(new Exception("Data is not a file"));
						return makeFileInFiles(acTup.getT2(), templateName, fileName, fileLocation, fileType,
								fileOverride, fileBytes);
					},
	...
	private Mono<FunctionOutput> processOutput(Map<String, Object> fileDetails) {
		if (fileDetails == null) {
			return Mono.just(new FunctionOutput(
					List.of(EventResult
							.outputOf(Map.of(EVENT_DATA, new JsonObject())))));
		}

		return Mono.just(new FunctionOutput(
				List.of(EventResult
						.outputOf(Map.of(EVENT_DATA, gson.toJsonTree(fileDetails))))));
	}

	private Mono<Map<String, Object>> makeFileInFiles(String clientCode, String templateName, String fileName,
			String fileLocation, String fileType,
			boolean override, byte[] fileBytes) {
		try {
