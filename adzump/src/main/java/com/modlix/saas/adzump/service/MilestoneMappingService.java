...
/**
 * Account-default / per-campaign milestone-mapping config. Same shape as the performance-policy
 * and autonomy services (the account default additionally carries a {@code productTemplateId}).
 * Every tenant-scoped method resolves an effective client code (see {@link CampaignPlanService}
 * for the rule, which mirrors {@code files}); scope is forced server-side and never trusted from
 * the request body.
 */
@Service
public class MilestoneMappingService {
    ...
    private final MilestoneMappingDao dao;
    ...
    public MilestoneMappingService(MilestoneMappingDao dao, CampaignPlanService campaignPlanService,
    ...
    // TODO(J5): fall back to the vertical-pack / product-template default when neither row exists.
    // No @PreAuthorize: tenant-scoped to the effective client below.
    public MilestoneMapping getEffective(ULong campaignId, String targetClientCode) {
        ...
        if (campaignId != null) {
            MilestoneMapping override = this.findOne(clientCode, AdzumpMilestoneMappingScope.CAMPAIGN_OVERRIDE,
                    campaignId);
            if (override != null)
                return override;
        }

        return this.findOne(clientCode, AdzumpMilestoneMappingScope.ACCOUNT_DEFAULT, null);
    }

    @PreAuthorize("hasAuthority('Authorities.ROLE_Owner')")
    public MilestoneMapping putAccountDefault(MilestoneMapping mapping, String targetClientCode) {

        ContextAuthentication ca = SecurityContextUtil.getUsersContextAuthentication();

        if (mapping.getScope() != null && mapping.getScope() != AdzumpMilestoneMappingScope.ACCOUNT_DEFAULT)
            msgService.throwMessage(msg -> new GenericException(HttpStatus.BAD_REQUEST, msg),
                    AdzumpMessageResourceService.INVALID_SCOPE, mapping.getScope());
        ...
        mapping.setScope(AdzumpMilestoneMappingScope.ACCOUNT_DEFAULT);
        mapping.setClientCode(clientCode);
        mapping.setCampaignId(null);

        MilestoneMapping existing = this.findOne(clientCode, AdzumpMilestoneMappingScope.ACCOUNT_DEFAULT, null);
        ...
        existing.setBody(mapping.getBody());
        existing.setProductTemplateId(mapping.getProductTemplateId());
    ...
    public MilestoneMapping putCampaignOverride(ULong campaignId, JsonNode partialBody) {
        ...
        MilestoneMapping existing = this.findOne(clientCode, AdzumpMilestoneMappingScope.CAMPAIGN_OVERRIDE, campaignId);

        if (existing == null) {

            MilestoneMapping override = new MilestoneMapping()
                    .setClientCode(clientCode)
                    .setScope(AdzumpMilestoneMappingScope.CAMPAIGN_OVERRIDE)
    ...
    private MilestoneMapping findOne(String clientCode, AdzumpMilestoneMappingScope scope, ULong campaignId) {
        ...
        List<MilestoneMapping> rows = this.dao.readAll(
