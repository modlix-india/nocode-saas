...
public abstract class AbstractSignatureFunction extends AbstractReactiveFunction {
    ...
    protected Schema getAlgorithms() {
        return Schema.ofString(ALGORITHM)
                .setEnums(
                        SignatureAlgo.getAvailableAlgos(SignatureAlgo.HS256, SignatureAlgo.HS384, SignatureAlgo.HS512));
    }

    protected Schema getCharsets() {
        return Schema.ofString(CHARSET)
                .setEnums(List.of(
                        new JsonPrimitive(StandardCharsets.US_ASCII.name()),
    ...
    protected Schema getEncoders() {
        return Schema.ofString(SIGNATURE_ENCODER)
                .setEnums(StringEncoder.getAvailableEncoder())
    ...
    protected Mono<FunctionOutput> sign(
        ...
        return reactiveSigner
                .sign(this.toBytes(data, charset))
                .map(encoder::encode)
                .map(encoded -> new FunctionOutput(
                        List.of(EventResult.outputOf(Map.of(EVENT_DATA, new JsonPrimitive(encoded))))));
    }
    ...
    protected Mono<FunctionOutput> isValid(
        ...
        return reactiveSigner
                .isValid(this.toBytes(data, charset), encoder.decode(signature))
                .map(isValid -> new FunctionOutput(
                        List.of(EventResult.outputOf(Map.of(IS_VALID, new JsonPrimitive(isValid))))));
    }

    protected byte[] toBytes(JsonElement data, Charset charset) {
        return data.isJsonPrimitive()
                ? this.toBytes(data.getAsString(), charset)
