...
public class MongoAppDataService extends RedisPubSubAdapter<String, String> implements IAppDataService {
        ...
        private <T> Mono<T> mongoObjectNotFound(String messageId, Object... params) {
                return this.msgService.throwMessage(
                                msg -> new StorageObjectNotFoundException(HttpStatus.NOT_FOUND, msg), messageId,
                                params);
        }
        ...
        public Mono<Map<String, Object>> create(Connection conn, Storage storage, DataObject dataObject) {
                return FlatMapUtil.flatMapMonoWithNull(
                                ...
                                (ca, collection, schema, appSchemaRepo) -> this.handleRelationsAndValidate(
                                                dataObject.getData(), storage, schema, appSchemaRepo),
                                (ca, collection, schema, appSchemaRepo,
                                                je) -> Mono.from(collection.insertOne(BJsonUtil.from(
                ...
                return FlatMapUtil.flatMapMonoWithNull(
                                ...
                                (ca, collection, schema, appSchemaRepo) -> {
                                        ...
                                        return Mono.from(collection
                                                        ...
                                                        .flatMap(oDocument -> DifferenceApplicator
                                                                        .apply(overridableObject, oDocument))
                                                        .contextWrite(Context.of(LogUtil.METHOD_NAME,
                                                                        "MongoAppDataService.update"));
                                },
                                (ca, collection, schema, appSchemaRepo, overridableObject) -> this
                                                .handleRelationsAndValidate(
                                                                (Map<String, Object>) overridableObject, storage,
                                                                schema, appSchemaRepo),
                                (ca, collection, schema, appSchemaRepo, overridableObject,
                                                je) -> Mono.from(collection.replaceOne(
                                                                ...
                                                                BJsonUtil.from(
                                                                                storage.getRelations() != null
                                                                                                ...
                                                                                                : Set.of(),
                                                                                je))),
                                (ca, collection, schema, appSchemaRepo, overridableObject, je, result) -> Mono
                                                .from(collection
                                                                .find(Filters.eq(ID, objectId))
                                                                .first())
                                                .switchIfEmpty(this.mongoObjectNotFound(
                                                                ...
                                                                storage.getName(),
                                                                key)),
                                (ca, collection, scheme, appSchemaRepo, overridableObject, je, result, doc) -> this
                                                .addVersion(conn, storage, dataObject.getMessage(), objectId, ca, doc,
                                                                "UPDATE"),
        ...
        private Mono<JsonObject> handleRelationsAndValidate(
                        Map<String, Object> objectMap, Storage storage, Schema schema,
        ...
        private Mono<InsertOneResult> addVersion(
                ...
                if (BooleanUtil.safeValueOf(storage.getIsVersioned()))
                        versionDocument.append(OBJECT, document);
        ...
        public Mono<Map<String, Object>> read(Connection conn, Storage storage, String id) {
                ...
                return FlatMapUtil.flatMapMono(() -> this.getCollection(conn, storage), collection -> Mono.from(
                                collection.find(Filters.eq(ID, objectId)).first())
                                .map(doc -> this.convertBisonIds(storage, doc, Boolean.FALSE)))
                                .contextWrite(Context.of(LogUtil.METHOD_NAME, "MongoAppDataService.read"))
                                .switchIfEmpty(this.mongoObjectNotFound(
                                                AbstractMongoMessageResourceService.OBJECT_NOT_FOUND, storage.getName(),
                                                id));
        }
        ...
        public Mono<Page<Map<String, Object>>> readPage(Connection conn, Storage storage, Query query) {
                ...
                return FlatMapUtil.flatMapMono(
                                () -> this.getCollection(conn, storage),
                                collection -> this.filter(storage, condition),
                                (collection, bsonCondition) -> this
        ...
        public Flux<Map<String, Object>> readPageAsFlux(Connection conn, Storage storage, Query query) {
                ...
                return this.getCollection(conn, storage)
                                .flatMapMany(collection -> this.filter(storage, condition)
                                                .flatMapMany(bsonCondition -> this
                                                                .applyQueryOnElements(collection, query, bsonCondition,
                                                                                page)
                                                                .map(document -> this.convertBisonIds(storage, document,
                                                                                Boolean.FALSE))))
        ...
        public Mono<Page<Map<String, Object>>> readPageVersion(
                ...
                return FlatMapUtil.flatMapMono(
                                () -> this.getVersionCollection(conn, storage),
                                ...
                                (vCollection, bsonCondition, list) -> BooleanUtil.safeValueOf(count)
                                                ...
                                                : Mono.just(page.getOffset() + list.size()),
                                (vCollection, bsonCondition, list, cnt) -> Mono
                                                .just(PageableExecutionUtils.getPage(list, page, cnt::longValue)))
        ...
        private Mono<Bson> filterConditionFilter(Storage storage, FilterCondition fc) { // NOSONAR
                ...
                if (fc.getOperator() == IS_NULL)
                        return fc.isNegate()
                                        ...
                                        : Mono.just(Filters.or(Filters.eq(fc.getField(), null),
                                                        Filters.exists(fc.getField(), false)));

                Object value = fc.getValue();
                boolean isObjectIdField = (fc.getField().equals("_id")
                                || (storage.getRelations() != null
                                                && storage.getRelations().containsKey(fc.getField())));
                if (isObjectIdField && value != null)
                        value = new ObjectId(value.toString());

                if (fc.getOperator() == IN) {
                        if (value == null
                                        && (fc.getMultiValue() == null || fc.getMultiValue().isEmpty()))
                                return Mono.empty();
                ...
                if (fc.getOperator() == FilterConditionOperator.MATCH) {
                        ...
                        Document doc = new Document(Map.of(
                                        ...
                                        Map.of(
                                                        "$elemMatch",
                                                        Map.of(
                                                                        CommonsUtil.nonNullValue(FILTER_MATCH_OPERATOR
                                                                                        .get(fc.getMatchOperator()),
                                                                                        "$eq"),
                                                                        value))));
                        return Mono.just(doc);
                }
                ...
                if (fc.getOperator() == FilterConditionOperator.MATCH_ALL) {
                        ...
                        Document doc = new Document(
                                        Map.of(fc.getField(), Map.of("$all",
                                                        value == null ? fc.getMultiValue() : List.of(value))));
        ...
        private Mono<MongoCollection<Document>> getCollection(Connection conn, Storage storage) {
        ...
        private Mono<MongoCollection<Document>> getVersionCollection(Connection conn, Storage storage) {
                return SecurityContextUtil.getUsersContextAuthentication()
        ...
        private Mono<Boolean> manageIndexes(
                        MongoCollection<Document> collection, Map<String, StorageIndex> indexes,
                        List<String> textIndexFields) {
                return FlatMapUtil.flatMapMonoWithNull(
                                                        ...
                                                        .flatMap(e -> {
                                                                ...
                                                                List<Bson> bDocs = ins.getFields().stream()
                                                                                .map(si -> si.getDirection() == Direction.ASC
                                                                                                ? Indexes.ascending(si
                                                                                                                .getFieldName())
                                                                                                : Indexes.descending(si
                                                                                                                .getFieldName()))
                                                                                .toList();

                                                                Bson ind = bDocs.size() == 1 ? bDocs.getFirst()
                                                                                : Indexes.compoundIndex(bDocs);
                                ...
                                .contextWrite(Context.of(LogUtil.METHOD_NAME,
                                                "MongoAppDataService.getCollection (Cache Supplier)"));
        }
