...
public class Storage extends AbstractOverridableDTO<Storage> {
    ...
    private void subMakeOverride(Storage base, Storage obj) {
        ...
        if (EqualsUtil.safeEquals(obj.createAuth, base.createAuth)) obj.createAuth = null;

        if (EqualsUtil.safeEquals(obj.readAuth, base.readAuth)) obj.readAuth = null;

        if (EqualsUtil.safeEquals(obj.updateAuth, base.updateAuth)) obj.updateAuth = null;

        if (EqualsUtil.safeEquals(obj.deleteAuth, base.deleteAuth)) obj.deleteAuth = null;

        if (EqualsUtil.safeEquals(obj.uniqueName, base.uniqueName)) obj.uniqueName = null;

        if (EqualsUtil.safeEquals(obj.isAppLevel, base.isAppLevel)) obj.isAppLevel = null;

        if (EqualsUtil.safeEquals(obj.onlyThruKIRun, base.onlyThruKIRun)) obj.onlyThruKIRun = null;

        if (EqualsUtil.safeEquals(obj.generateEvents, base.generateEvents)) obj.generateEvents = null;
    }
