...
public class GetClient extends AbstractReactiveFunction {
    ...
    protected Mono<FunctionOutput> internalExecute(ReactiveFunctionExecutionParameters context) {
        ...
        return Mono.deferContextual(cv -> {
            ...
            Mono<Client> client = param.isString()
                    ? this.securityService.getClientByCode(param.getAsString())
                    : this.securityService.getClientById(BigInteger.valueOf(param.getAsLong()));

            return client.map(c -> new FunctionOutput(
                    List.of(EventResult.outputOf(Map.of(EVENT_DATA_CLIENT, this.gson.toJsonTree(c, Client.class))))));
        });
    }
}
