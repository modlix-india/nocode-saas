...
public class AppDAO extends AbstractUpdatableDAO<SecurityAppRecord, ULong, App> {
    ...
    public Mono<Page<Client>> readSystemClientByAppCode(org.springframework.data.domain.Pageable pageable, ULong appId,
            boolean onlyWriteAccess, String name) {

        Condition nameFilterCond = DSL
                .and(!name.isEmpty() ? SECURITY_CLIENT.NAME.like("%" + name + "%") : DSL.trueCondition());

        Condition writeAccessCond = DSL
                .and(onlyWriteAccess ? SECURITY_APP_ACCESS.EDIT_ACCESS.eq(UByte.valueOf(1)) : DSL.trueCondition());

        Mono<Integer> recordsCount = Mono.from(
                this.dslContext.selectCount().from(this.dslContext.select(SECURITY_CLIENT.fields())
                        .from(SECURITY_CLIENT).join(SECURITY_APP_ACCESS)
                        .on(SECURITY_APP_ACCESS.CLIENT_ID.eq(SECURITY_CLIENT.ID))
                        .where(SECURITY_APP_ACCESS.APP_ID.eq(appId).and(nameFilterCond).and(writeAccessCond))
                        .union(this.dslContext.select(SECURITY_CLIENT.fields())
                                .from(SECURITY_CLIENT).join(SECURITY_APP)
                                .on(SECURITY_APP.CLIENT_ID.eq(SECURITY_CLIENT.ID))
                                .where(SECURITY_APP.ID.eq(appId).and(nameFilterCond)))))
                .map(Record1::value1);

        Mono<List<Client>> recordsList = Flux.from(
                this.dslContext.select(SECURITY_CLIENT.fields())
                        .from(SECURITY_CLIENT).join(SECURITY_APP_ACCESS)
                        .on(SECURITY_APP_ACCESS.CLIENT_ID.eq(SECURITY_CLIENT.ID))
                        .where(SECURITY_APP_ACCESS.APP_ID.eq(appId).and(nameFilterCond).and(writeAccessCond))
                        .union(this.dslContext.select(SECURITY_CLIENT.fields())
    ...
    public Mono<Page<Client>> readClientByAppCode(org.springframework.data.domain.Pageable pageable, ULong appId,
            boolean onlyWriteAccess, String name, ULong clientId) {
        ...
        Condition managingCond = DSL.and(SECURITY_CLIENT_MANAGE.MANAGE_CLIENT_ID.eq(clientId)
                .or(SECURITY_APP_ACCESS.CLIENT_ID.eq(clientId)));
        ...
        Mono<Integer> recordsCount = Mono.from(
                this.dslContext.selectCount().from(this.dslContext.select(SECURITY_CLIENT.fields())
                        .from(SECURITY_CLIENT).join(SECURITY_APP_ACCESS)
                        .on(SECURITY_APP_ACCESS.CLIENT_ID.eq(SECURITY_CLIENT.ID))
                        .leftOuterJoin(SECURITY_CLIENT_MANAGE)
                        .on(SECURITY_CLIENT_MANAGE.CLIENT_ID.eq(SECURITY_APP_ACCESS.CLIENT_ID))
                        .where(SECURITY_APP_ACCESS.APP_ID.eq(appId)
                                .and(managingCond)
                                .and(nameFilterCond).and(writeAccessCond))
                        .union(this.dslContext.select(SECURITY_CLIENT.fields())
                                .from(SECURITY_CLIENT).join(SECURITY_APP)
                                .on(SECURITY_APP.CLIENT_ID.eq(SECURITY_CLIENT.ID))
                                .where(SECURITY_APP.ID.eq(appId).and(SECURITY_APP.CLIENT_ID.eq(clientId))
                                        .and(nameFilterCond)))))
                .map(Record1::value1);
        ...
        Mono<List<Client>> recordsList = Flux.from(
                this.dslContext.select(SECURITY_CLIENT.fields())
                        ...
                        .leftOuterJoin(SECURITY_CLIENT_MANAGE)
                        .on(SECURITY_CLIENT_MANAGE.CLIENT_ID.eq(SECURITY_APP_ACCESS.CLIENT_ID))
                        .where(SECURITY_APP_ACCESS.APP_ID.eq(appId).and(managingCond).and(nameFilterCond)
                                .and(writeAccessCond))
                        .union(this.dslContext.select(SECURITY_CLIENT.fields())
                                .from(SECURITY_CLIENT).join(SECURITY_APP)
                                .on(SECURITY_APP.CLIENT_ID.eq(SECURITY_CLIENT.ID))
