...
public class SSOBundleDAO extends AbstractUpdatableDAO<SecurityAppSsoBundleRecord, ULong, SSOBundle> {
    ...
    public Mono<SSOBundle> readById(ULong id) {
        return FlatMapUtil.flatMapMono(
                ...
                bundle -> Flux
                        .from(this.dslContext.selectFrom(SECURITY_BUNDLED_APP)
                                .where(SECURITY_BUNDLED_APP.BUNDLE_ID.eq(id)))
    ...
    public <A extends AbstractUpdatableDTO<ULong, ULong>> Mono<SSOBundle> update(A entity) {
        ...
        return FlatMapUtil.flatMapMono(
                ...
                updated -> Mono.from(this.dslContext.deleteFrom(SECURITY_BUNDLED_APP)
                        .where(SECURITY_BUNDLED_APP.BUNDLE_ID.eq(bundle.getId()))),
    ...
    public Mono<SSOBundle> createBundledApps(SSOBundle newOne, SSOBundle bundle) {
        return FlatMapUtil.flatMapMono(
                () -> {
                    if (bundle.getApps() == null || bundle.getApps().isEmpty())
                    ...
                    return Flux.fromIterable(bundle.getApps())
                            .flatMap(e -> Mono.from(this.dslContext.insertInto(SECURITY_BUNDLED_APP,
                                    ...
                                    .values(newOne.getId(), bundle.getCreatedBy(), e.getAppCode(), e.getAppUrlId())
                                    .returning(SECURITY_BUNDLED_APP.ID)))
                ...
                appListIds -> Flux
                        .from(this.dslContext.selectFrom(SECURITY_BUNDLED_APP)
                                .where(SECURITY_BUNDLED_APP.ID.in(appListIds)))
                        .map(rec -> rec.into(SSOBundle.SSOBundledApp.class))
        ...
        return FlatMapUtil.flatMapMono(
                () -> Flux
                        .from(this.dslContext
                                .select(SECURITY_APP_SSO_BUNDLE.ID, SECURITY_APP_SSO_BUNDLE.BUNDLE_NAME,
                                        SECURITY_APP_SSO_BUNDLE.CLIENT_CODE)
                                .from(SECURITY_BUNDLED_APP)
                                .leftJoin(SECURITY_APP_SSO_BUNDLE)
                                .on(SECURITY_APP_SSO_BUNDLE.ID.eq(SECURITY_BUNDLED_APP.BUNDLE_ID))
                                .where(SECURITY_APP_SSO_BUNDLE.CLIENT_CODE.eq(clientCode)
                                        .and(SECURITY_BUNDLED_APP.APP_CODE.eq(appCode))))
                        .map(rec -> rec.into(SSOBundle.class)).collectMap(SSOBundle::getId, Function.identity()),

                (Map<ULong, SSOBundle> bundles) -> Flux
                        .from(this.dslContext.selectFrom(SECURITY_BUNDLED_APP)
                                .where(SECURITY_BUNDLED_APP.BUNDLE_ID.in(bundles.keySet())))
                        .map(rec -> rec.into(SSOBundle.SSOBundledApp.class))
                        .collectMultimap(SSOBundle.SSOBundledApp::getBundleId)
                        .map(bundledApps -> {
                            ...
                            for (Map.Entry<ULong, Collection<SSOBundle.SSOBundledApp>> entry : bundledApps.entrySet()) {
                                ...
                                if (!bundles.containsKey(list.getFirst().getBundleId()))
                                    continue;
