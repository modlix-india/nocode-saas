...
public class AppService extends AbstractJOOQUpdatableDataService<SecurityAppRecord, ULong, App, AppDAO> {
    ...
    public Mono<Page<Client>> getAppClients(Pageable pageable, String appCode, boolean onlyWriteAccess, String name) {
        ...
        return FlatMapUtil.flatMapMono(
                ...
                (ca, app) -> {
                    ...
                    if (!ca.isSystemClient())
                        return this.dao.readClientByAppCode(pageable, app.getId(), onlyWriteAccess, name,
                                ULongUtil.valueOf(ca.getUser().getClientId()));

                    return this.dao.readSystemClientByAppCode(pageable, app.getId(), onlyWriteAccess, name);
                })
