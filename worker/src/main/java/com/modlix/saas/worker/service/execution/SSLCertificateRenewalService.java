...
public class SSLCertificateRenewalService extends AbstractExecutionService {
    ...
    private final IFeignSecurityService feignSecuritySSLService;

    public SSLCertificateRenewalService(IFeignSecurityService feignSecuritySSLService) {
    ...
    public String execute(Task task) {
        ...
        try {
            ...
            logger.info("SSL renewal complete — renewed: {}, failed: {}", result.getRenewedCount(), result.getFailedCount());
            return formatted;
        } catch (Exception e) {
