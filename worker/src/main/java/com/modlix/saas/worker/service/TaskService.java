...
public class TaskService extends AbstractJOOQUpdatableDataService<WorkerTasksRecord, ULong, Task, TaskDAO> {
    ...
    public void initializeTasksFromDatabase() {
        ...
        for (Task task : tasks) {
            try {
                this.quartzService.initializeTask(task);
                logger.info("Initialized task: {}", task.getName());
            } catch (Exception e) {
                logger.warn("Task already registered or failed to initialize: {} - {}", task.getName(), e.getMessage());
            }
        }
    }
