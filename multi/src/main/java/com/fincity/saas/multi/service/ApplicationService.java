            ...
            try (FileSystem zipfs = FileSystems.newFileSystem(URI.create("jar:" + modlFile.toUri().toString()),
                ...
                for (Path p : files) {
                    ...
                    try {
                        ...
                        // Read file content with UTF-8 encoding, handling potential encoding errors
                        String fileContent;
                        try {
                            fileContent = Files.readString(p);
                        } catch (Exception encodingEx) {
                            // If UTF-8 decoding fails, the file likely contains invalid UTF-8 or is binary
                        ...
                        Map<String, Object> m = this.objectMapper.readValue(fileContent, Map.class);
                        ...
                        // Skip files that can't be processed as JSON
                        // This handles cases where files are binary, corrupted, or not JSON
                        continue;
                    }
                }
            }
