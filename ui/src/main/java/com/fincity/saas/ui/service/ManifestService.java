...
public class ManifestService {
	...
	public Mono<ObjectWithUniqueID<String>> getManifest(String appCode, String clientCode) {
		...
		return cacheService.cacheValueOrGet(this.appService.getCacheName(appCode + "_" + CACHE_NAME_MANIFEST, appCode),
				() -> FlatMapUtil
						.flatMapMonoWithNull(() -> appService.read(appCode, appCode, clientCode),
								app -> Mono.just(new ObjectWithUniqueID<String>(this.manifestFromApp(app))))
						.contextWrite(Context.of(LogUtil.METHOD_NAME, "ManifestService.getManifest")),
				clientCode);
	}

	@SuppressWarnings("unchecked")
	public String manifestFromApp(ObjectWithUniqueID<Application> app) {

		if (app == null || app.getObject().getProperties() == null)
			return "";

		Map<String, Object> props = app.getObject().getProperties();
