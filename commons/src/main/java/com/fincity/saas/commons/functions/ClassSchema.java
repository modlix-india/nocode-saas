...
public class ClassSchema {
        ...
        for (PackageMapping mapping : packageConfig.getPackageMappings()) {
            if (packageName.startsWith(mapping.getPackagePrefix())) {
                if (mapping.isAppendSubpackages()) {
                    return buildNamespaceFromPackage(
                            packageName, mapping.getPackagePrefix(), mapping.getBaseNamespace());
                } else {
    ...
    private String buildNamespaceFromPackage(String fullPackageName, String basePackage, String baseNamespace) {
        ...
        return baseNamespace
                + Arrays.stream(subPackage.split("\\."))
                        .filter(s -> !s.isEmpty())
                        .map(this::capitalize)
                        .map(s -> "." + s)
    ...
    public static class PackageConfig {
        @Builder.Default
        private String serviceName = "Commons";

        @Builder.Default
        private String commonsPackagePrefix = "com.fincity.saas.commons";

        @Builder.Default
        private String commonsNamespace = "Commons";

        @Builder.Default
        private String defaultNamespace = "Common";

        @Builder.Default
        private List<PackageMapping> packageMappings = List.of();
    ...
    public static class PackageMapping {
        ...
        @Builder.Default
        private boolean appendSubpackages = true;
