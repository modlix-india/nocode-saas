...
public class JsonUnflattener {
    ...
    public static JsonObject unflatten(Map<String, String> flatList, Map<String, Set<SchemaType>> flattenedSchemaType) {
        ...
        for (Entry<String, String> entry : flatList.entrySet()) {
            ...
            for (; i < parts.length - 1; i++) {
                ...
                if (part.indexOf('[') == -1) pointer = processObjectOperator(pointer, part);
                else pointer = processArrayOperator(pointer, part);
            }
    ...
    private static JsonElement processObjectOperator(JsonElement pointer, String part) {
        ...
        if (!pointer.isJsonObject()) throw new GenericException(HttpStatus.INTERNAL_SERVER_ERROR, OBJECT_NOT_FOUND);

        JsonObject mini = pointer.getAsJsonObject();

        if (!mini.has(part)) mini.add(part, new JsonObject());
    ...
    private static JsonElement processArrayOperator(JsonElement pointer, String part) {
        ...
        if (!mini.has(subParts[j])) mini.add(subParts[j], new JsonArray());

        pointer = mini.get(subParts[j]);

        for (j = 1; j < subParts.length - 1; j++) {

            if (!pointer.isJsonArray()) throw new GenericException(HttpStatus.INTERNAL_SERVER_ERROR, ARRAY_NOT_FOUND);
            ...
            if (index >= miniArray.size()) {
                ...
                for (int k = 0; k < (index - size); k++) miniArray.add(new JsonArray());
            }
        ...
        if (index >= miniArray.size()) {
            ...
            int size = miniArray.size();
            for (int k = 0; k <= (index - size); k++) miniArray.add(new JsonObject());
        }
    ...
    private static void processArrayOperatorForPlaceValue(
            String part, JsonElement pointer, String value, Set<SchemaType> schemaTypes) {
        ...
        if (!mini.has(subParts[j])) mini.add(subParts[j], new JsonObject());
        ...
        if (index >= miniArray.size()) {
            ...
            int size = miniArray.size();
            for (int k = 0; k < (index - size); k++) miniArray.add(JsonNull.INSTANCE);
        }
    ...
    private static JsonElement getJSONElementBySchemaType(Set<SchemaType> schemaTypes, String value) {
        ...
        if (StringUtil.safeIsBlank(value) || "null".equalsIgnoreCase(value)) return JsonNull.INSTANCE;

        if (schemaTypes.contains(SchemaType.STRING)) return new JsonPrimitive(value);
